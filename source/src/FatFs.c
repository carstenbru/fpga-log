...
)
{
	...
	return mul34_17(clst, fs->csize) + fs->database;
}
...
)
{
	...
	dp->sect = sect + cast_to_ulong(idx / (SS(dp->fs) / SZ_DIR));					/* Sector# of the directory entry */
...
)
{
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	fasize = cast_to_ulong(LD_WORD(fs->win+BPB_FATSz16));				/* Number of sectors per FAT */
	...
	if (fs->n_fats != 1 && fs->n_fats != 2)				/* (Must be 1 or 2) */
		return FR_NO_FILESYSTEM;
	fasize *= cast_to_ulong(fs->n_fats);								/* Number of sectors for FAT area */
	...
	nclst = (tsect - sysect) / cast_to_ulong(fs->csize);				/* Number of clusters */
